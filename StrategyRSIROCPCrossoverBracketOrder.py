...
class StrategyRSIROCPCrossoverBracketOrder(StrategyBase):
    ...
    @staticmethod
    def name():
        return 'RSI and (ROCP for exit) Crossover Bracket Order Strategy'
    ...
    def GannHilo(self, high, low, close, high_length=None, low_length=None):
        ...
        #hilo = talib.pd.Series(talib._ta_lib.numpy.NaN, index=close.index)
    ...
    def strategy_select_instruments_for_entry(self, candle, instruments_bucket):
        ...
        for instrument in instruments_bucket:
            rsi_cv, rsi = self.get_rsi_crossover_value(instrument)
            #rocp_cv = self.get_rocp_crossover_value(instrument)

            if ((rsi_cv == 1) & ((rsi.iloc[-1] > rsi.shift(2).iloc[-1]) & (rsi.iloc[-1] > rsi.shift(1).iloc[-1]))):
                selected_instruments_bucket.append(instrument)
                sideband_info_bucket.append({'action': 'BUY'})
            elif ((rsi_cv == -1) & ((rsi.iloc[-1] < rsi.shift(2).iloc[-1]) & (rsi.iloc[-1] < rsi.shift(1).iloc[-1]))):
